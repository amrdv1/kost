...
app.get('/api/dev/seed', async (req, res) => {
  try {
    ...
    await pool.query(`
      ...
      VALUES ('Степан', 'Бро, давай рви чарти!', 150.00, 'UAH', 'PAID', 'APPROVED')
    `);

    // Ordinary donation 2
    await pool.query(`
      INSERT INTO donations (customer_name, message, amount, currency, payment_status, audio_status)
      VALUES ('Анонім', 'Просто на каву', 50.00, 'UAH', 'PAID', 'APPROVED')
    `);
    ...
    res.send('Database seeded with dummy donations (correct statuses)!');
  } catch (err) {
    console.error(err);
